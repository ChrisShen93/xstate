...
- 符合 [W3C SCXML Specification](https://www.w3.org/TR/scxml/) 规范和 David Harel 的状态图范式
- 基于事件模型的 [Actor 模型](https://en.wikipedia.org/wiki/Actor_model)
- 高适配性：框架、平台无关
- 状态机可以被完全序列化为 JSON （和 SCXML）
- 纯函数 `Machine(...)` API
- 零依赖

## 为什么选择 XState

如果你正在决定是否要使用 XState，[John Yanarella](https://github.com/CodeCatalyst) 总结了他的理由：
